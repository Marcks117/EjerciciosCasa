...
int screenW = 0.0;
int screenH = 0.0;
float angf = 0.0, Muñeca = 0.0, angPu = 0.0, angD = 0.0;
//NEW//////////////////NEW//////////////////NEW//////////////////NEW////////////////
static GLuint ciudad_display_list;	//Display List for the Monito


//NEW// Keyframes
//Variables de dibujo y manipulacion del monito
float posX =0, posY = 2.5, posZ =-3.5, rotRodIzq = 0, rotRodDer = 0; //Guardar posiciones en variables
float cabeza = 0, BrazoDer = 0, BrazoIzq = 0, Cadera = 0;
float giroMonito = 0;

//Variables para la linea de tiempo de animación
#define MAX_FRAMES 10
int i_max_steps = 60; //90
int i_curr_steps = 0;
...
typedef struct _frame
{
	...
	float posZ;		//Variable para PosicionZ
 	float incX;		//Variable para IncrementoX (interpolación)
	float incY;		//Variable para IncrementoY (interpolación)
	float incZ;		//Variable para IncrementoZ (interpolación)
...
void saveFrame ( void )
{
	...
	KeyFrame[FrameIndex].angD = angD;

	//Asignacion del estado actual a las variables
	KeyFrame[FrameIndex].posX=posX;
	KeyFrame[FrameIndex].posY=posY;
	KeyFrame[FrameIndex].posZ=posZ;

	KeyFrame[FrameIndex].angPu= angPu;
	KeyFrame[FrameIndex].angf= angf;
...
void resetElements( void )
{
	...
	rotRodDer = KeyFrame[0].angD;

	rotRodIzq=KeyFrame[0].angPu;
	giroMonito=KeyFrame[0].angf;
...
void interpolation ( void )
{
	...
	KeyFrame[playIndex].angf = (KeyFrame[playIndex + 1].angf - KeyFrame[playIndex].angf) / i_max_steps;

	KeyFrame[playIndex].angDInc = (KeyFrame[playIndex + 1].angD - KeyFrame[playIndex].angD) / i_max_steps;

//	KeyFrame[playIndex].BrazoDerInc = (KeyFrame[playIndex + 1].BrazoDer - KeyFrame[playIndex].BrazoDer) / i_max_steps;
//	KeyFrame[playIndex].BrazoIzqInc = (KeyFrame[playIndex + 1].BrazoIzq - KeyFrame[playIndex].BrazoIzq) / i_max_steps;

//	KeyFrame[playIndex].cabezaInc = (KeyFrame[playIndex + 1].cabeza - KeyFrame[playIndex].cabeza) / i_max_steps;

//	KeyFrame[playIndex].CaderaInc = (KeyFrame[playIndex + 1].Cadera - KeyFrame[playIndex].Cadera) / i_max_steps;
}
...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	//NEW Iniciar variables de KeyFrames, inicializar en cero todas las variables
	for(int i=0; i<MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].angf =0;
		KeyFrame[i].angfInc =0;

		KeyFrame[i].angD = 0;
		KeyFrame[i].angDInc = 0;
	}
	//NEW//////////////////NEW//////////////////NEW///////////
...
void animacion()
{
	...
	{		
		...
		else
		{
			...
			angf+=KeyFrame[playIndex].angfInc;

			angD += KeyFrame[playIndex].angDInc;
